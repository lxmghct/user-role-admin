...
public class UserUtils {
     ...
     * @param systemId              系统id
     */
    public static boolean checkUserHasPermissionForSystem(List<Map<String, Object>> roleAndPermissionInfo, Integer systemId) {
        if (roleAndPermissionInfo != null && roleAndPermissionInfo.size() > 0) {
            Map<String, Object> tempInfo = roleAndPermissionInfo.get(0);
            String systemString = (String) tempInfo.get("systemList");
            if (systemString != null) {
                for (String s : systemString.split(",")) {
                    if (s.equals(String.valueOf(systemId))) {
                        return true;
                    }
                }
            }
    ...
    public static boolean updateUserAvatar(User user, MultipartFile avatar) {
        ...
        if (!avatarName.equals(user.getAvatar())) {
            if (user.getAvatar() != null) {
                if (!FileUtils.deleteFileOrDirectory(StorageEnum.USER_AVATAR_PATH.getDesc() + user.getAvatar())) {
                    return false;
                }
            }
     ...
     * @param systemId 系统id
     */
    public static ResponseVO<Map<String, Object>> handleUserLogin(User user, String password, Integer systemId) {
        if (SystemEnum.getNameById(systemId) == null) {
            return ResponseVO.error(StatusEnum.SYSTEM_NOT_EXISTS);
        }
        ...
        if (!checkUserHasPermissionForSystem(roleAndPermissionInfo, systemId)) {
            return ResponseVO.error(StatusEnum.PERMISSION_DENIED_FOR_SYSTEM);
        }
        String ip = ServletUtils.getRequest().getHeader("X-Real-IP");
        instance.loginLogService.save(new LoginLog(user.getId(), ip, new Date(), systemId));
